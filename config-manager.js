...
class ConfigManager {
        ...
        pages.forEach((p, idx) => {
            ...
            card.innerHTML = `
                <div class="preview-page-header">
                    ${document.getElementById('show-page-numbers')?.checked !== false
        ...
        switch (page.type) {
            ...
            case 'worship':
                return (c.songs || []).length
                    ...
                    : '<div class="no-items">暂无歌曲</div>';
            case 'announcements':
                return (c.items || []).length
                    ? c.items.map(it => `<div class="announcement-item ${it.important ? 'important' : ''}">
                        <strong>${it.title || '未命名报告'}</strong>
                        <div>${it.content || ''}</div>
                    </div>`).join('')
                    : '<div class="no-items">暂无家事报告</div>';
            case 'scripture':
                return c.scripture
                    ? `<div>
                        <div class="scripture-text" style="font-size:1.2em">${c.scripture.text || ''}</div>
                        <div style="opacity:0.8">${c.scripture.book || ''} ${c.scripture.chapter || ''}:${c.scripture.verse || ''}</div>
                    </div>`
                    : '<div class="no-items">未设置经文</div>';
            case 'message':
                return c.message
                    ? `<div>
                        <div><strong>题目：</strong>${c.message.title || ''}</div>
                        <div><strong>讲员：</strong>${c.message.speaker || ''}</div>
                        <div><strong>大纲：</strong><pre style="white-space:pre-wrap">${c.message.outline || ''}</pre></div>
                    </div>`
                    : '<div class="no-items">未设置信息分享</div>';
            case 'special':
                return `<div>
                    <div style="font-size:1.4em">${c.title || '特别项目'}</div>
                    <div style="margin:10px 0">${c.text || ''}</div>
                    ${c.scripture ? `<div style="font-style:italic">${c.scripture}</div>` : ''}
                </div>`;
            default:
                return '<div class="no-items">此页面类型暂不支持预览内容</div>';
        }
    }
    ...
    openPreview() {
        ...
        const pages = (this.currentData?.pages || [])
            .filter(p => p.enabled !== false)
            .sort((a, b) => (a.order || 0) - (b.order || 0));
        const container = win.document.getElementById('preview-area');
    ...
    ensureDataStructure() {
        ...
        if (Array.isArray(this.currentData.pages)) {
            this.currentData.pages.forEach(page => {
                ...
                if (page.type === 'scripture' && !page.content.scripture) {
                    page.content.scripture = {};
                }
                if (page.type === 'message' && !page.content.message) {
    ...
    getDefaultPages() {
        ...
        return [
            ...
            },
            {
                ...
                content: {
                    title: '祝福',
                    text: '愿主赐福给你，保护你；愿主使他的脸光照你，赐恩给你；愿主向你仰脸，赐你平安。',
                    scripture: '民数记 6:24-26'
                },
    ...
    addNewPage(pageType) {
        ...
        const newPage = {
            ...
            content: JSON.parse(JSON.stringify(template.content)), // 深度复制
    ...
    allPages.forEach((page) => {
        ...
        // ===== 修改点：复选框状态根据 enabled 设置 =====
        const isChecked = page.enabled !== false; // 默认为 true
